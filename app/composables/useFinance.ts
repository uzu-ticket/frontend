...
export function useFinance() {
  ...
  const selectedTransaction = ref<FinanceTransaction | null>(null);

  const activeWithdrawal = useState<WithdrawalDetails | null>(
    "finance:active-withdrawal",
    () => null,
  );
  ...
  async function fetchWallet() {
    ...
    try {
      const res = await instance.get(
        `/organisations/${activeOrgId.value}/wallet`,
      );
      const wallet = res.data?.data ?? res.data;
      if (wallet) {
        walletBalance.value = Number(wallet.balanceMinor ?? 0) / 100;
        pendingSettlement.value = Number(wallet.heldMinor ?? 0) / 100;
      }
    } catch {
  ...
  async function fetchTransactions() {
    ...
    try {
      const res = await instance.get(
        `/organisations/${activeOrgId.value}/wallet/transactions`,
      );
      const rawTransactions = res.data?.data ?? res.data;
      if (Array.isArray(rawTransactions)) {
        transactions.value = rawTransactions.map((tx: any) => ({
          id: tx.id,
          ...
          description:
            tx.description ||
            (tx.transactionType === "ticket_purchase"
              ? "Event ticket sales"
              : "Withdrawal"),
        ...
        totalDebit.value = transactions.value
          ...
          .reduce((sum, tx) => sum + tx.amount, 0);
        totalEarned.value = totalCredit.value;
      }
    } catch {
      transactions.value = [];
    }
  ...
  async function requestWithdrawal(data: {
    ...
    if (!activeOrgId.value)
      throw new Error("Select an organisation before withdrawing");
    const res = await instance.post(
      `/organisations/${activeOrgId.value}/withdrawals`,
      {
    ...
    const amount = Number(raw.amountMinor ?? 0) / 100;
    const withdrawal: WithdrawalDetails = {
      id: raw.id,
      amount,
      fee: 0,
      netAmount: amount,
      bankName: raw.bankName ?? "Settlement account",
      bankCode: raw.bankCode ?? "",
      accountNumber: raw.accountNumber ?? "",
      accountName: raw.accountName ?? "",
      referenceId: raw.pspTransferReference ?? raw.id,
      status: raw.status === "paid" ? "Completed" : "Processing",
      requestedAt: raw.requestedAt ?? new Date().toISOString(),
      timeline: [
        {
          title: "Withdrawal requested",
          timestamp: raw.requestedAt ?? "Just now",
          status: "completed",
        },
        {
          title: "Processing",
    ...
    activeWithdrawal.value = withdrawal;
  ...
  async function exportStatement() {
    ...
    try {
      const res = await instance.get(
        `/organisations/${activeOrgId.value}/withdrawals/statement.csv`,
        {
          responseType: "blob",
        },
      );
  ...
  function downloadCsvMock() {
    ...
    const rows = mockTransactions
      .map(
        (t) =>
          `"${t.date}","${t.description}","${t.type}","${t.amountFormatted}","${t.status}"`,
      )
      .join("\n");
